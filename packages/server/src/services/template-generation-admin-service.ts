...
function fallbackMetadata(input: {
  ...
  return {
    ...
    seoBody: [
      ...
      `При этом шаблон не жесткий: в нем можно менять длительность задач, состав этапов, сроки, связи между работами и подстраивать график под конкретный объект и темп работ.`,
    ].join('\n\n'),
    tags: [],
  };
}
  ...
  try {
    const response = await completeTextPrompt({
      ...
      prompt: [
        ...
        '- используй только заголовки ##;',
        '- текст должен быть готов к последующему рендерингу из Markdown в HTML;',
        '- seoBody должен состоять только из одного блока "## Что это за шаблон";',
        '- объем всего seoBody: примерно 300-500 символов;',
        ...
        'Первый абзац делай коротким и прикладным, примерно по длине этого образца: "Это готовый шаблон графика работ для [типа проекта]. Он собран в формате диаграммы Ганта в ГетГант и помогает сразу увидеть, из каких этапов состоит проект, что за чем идет и где нельзя ошибиться с очередностью. Вместо пустого листа у вас уже есть рабочая основа: [ключевые этапы из графика]. Такой шаблон удобно брать, когда нужно быстро разложить проект по шагам, оценить сроки, проверить зависимости и понять, какие контрольные точки стоит держать под рукой."',
        'В первом абзаце не углубляйся в логику графика, не расписывай длинные причинно-следственные цепочки и не уходи в подробные объяснения.',
        'Второй абзац делай очень коротким, по смыслу близким к формуле: "При этом шаблон не жесткий: в нем можно менять длительность задач, состав этапов, сроки, связи между работами и подстраивать график под конкретный объект, команду и темп работ."',
        'Не выходи за эту длину. Первый блок должен быть компактным.',
        '',
        'Стиль:',
        ...
        '- не используй H3 нигде в тексте;',
        '- не добавляй никакие другие секции кроме "## Что это за шаблон";',
        '- не превращай первый блок в длинную простыню текста;',
        '- не расписывай в первом блоке подробную логику графика;',
        '- первый блок должен быть примерно той же длины, что и пользовательский образец, без лишних рассуждений;',
        '- не добавляй секции "Как использовать шаблон", "Для чего нужен этот шаблон", "Какой состав и логика работ уже заложены", "Кому подойдет шаблон", "Если шаблон не подошел";',
        ...
        '- не превращай текст в отчет по графику: нужен очень короткий прикладной SEO-текст;',
