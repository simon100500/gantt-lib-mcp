  ...
  try {
    const response = await completeTextPrompt({
      ...
      prompt: [
        ...
        '',
        '## Почему удобно вести такой проект в GetGantt',
        'Объясни, почему именно в сервисе GetGantt удобно вести такой проект.',
        ...
        'Объясни, что в GetGantt удобно видеть последовательность работ, параллельные участки, дедлайны и контрольные точки, чтобы подрядчики и этапы не конфликтовали между собой.',
        ...
        'Заверши раздел мягким CTA в стиле: "Создайте свой график в GetGantt — ИИ адаптирует шаблон под ваш проект за 30 секунд."',
        'Длина раздела: 2-4 коротких абзаца.',
        '',
        'Стиль:',
        ...
        '- в seoBody должны быть только три секции: "Что это за шаблон", "Почему удобно вести такой проект в GetGantt", "Что дает этот шаблон";',
        ...
        '- раздел "Почему удобно вести такой проект в GetGantt" должен быть коротким, плотным и прикладным: максимум 2 абзаца;',
        '- в разделе "Почему удобно вести такой проект в GetGantt" пиши именно про удобство сервиса GetGantt: связи задач, наглядность сроков, контрольные точки, параллельные работы, а не про диаграмму Ганта в отрыве от продукта;',
