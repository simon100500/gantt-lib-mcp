  ...
  try {
    const response = await completeTextPrompt({
      ...
      prompt: [
        ...
        'Структура seoBody:',
        '## Что это за шаблон',
        '1-2 коротких абзаца. Этот блок должен быть компактным и самодостаточным, потому что на странице он будет показан сразу после графика.',
        'Объясни, что это за шаблон, для какого типа проекта он подходит, какую логику этапов помогает быстро увидеть.',
        'Не перечисляй подробно состав этапов через длинные списки: ниже на странице уже будет отдельный блок со структурой работ.',
        '',
        '## Почему удобно вести такой проект в GetGantt',
        '2-3 абзаца про ценность диаграммы Ганта, визуального контроля, дедлайнов, зависимостей задач и контрольных точек.',
        'Пиши как отдельную post-секцию после блока со структурой работ.',
        '',
        '## Что дает этот шаблон на старте',
