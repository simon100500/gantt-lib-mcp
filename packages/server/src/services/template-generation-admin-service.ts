...
function fallbackMetadata(input: {
  ...
  return {
    ...
    seoBody: [
      ...
      '- структуру source project;',
      '- сроки и этапы реализации;',
      '- зависимости задач и контрольные точки;',
      '- SEO-заголовок, описание и теги перед публикацией.',
    ].join('\n\n'),
    tags: [],
  };
}
...
async function generateMetadataDraft(input: {
  ...
  try {
    const response = await completeTextPrompt({
      ...
      maxTokens: 2200,
      prompt: [
        'Ты — senior SEO-копирайтер и практик проектного управления.',
        ...
        '- допускается **жирный** акцент внутри строк;',
        '- текст должен быть готов к последующему рендерингу из Markdown в HTML;',
        '- объем: 250-600 слов;',
        '- стиль: профессиональный, уверенный, живой, без канцелярита;',
        ...
        'Структура seoBody:',
        '## Что это за шаблон',
        '1-2 абзаца о назначении шаблона, его практической пользе и сценарии применения.',
        '',
        '## Что входит в график работ',
        'Короткий вводный абзац и затем список из 5-8 этапов с профессиональной терминологией.',
        '',
        '## Для кого подойдет шаблон',
        '2-4 сегмента аудитории с их выгодой.',
        '',
        '## Почему удобно вести такой проект в GetGantt',
        'Объясни ценность диаграммы Ганта, визуального контроля, дедлайнов, зависимостей задач и контрольных точек.',
        '',
        '## Что дает этот шаблон на старте',
        'Короткий завершающий блок с акцентом на экономию времени и порядок в проекте.',
        '',
