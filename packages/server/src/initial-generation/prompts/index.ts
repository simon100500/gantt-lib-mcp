...
export function buildStructurePrompt(input: PlannerPromptContext): string {
  return [
    ...
    'For Russian task titles, use construction naming in nominal operation form, not task-tracker imperative or infinitive phrasing.',
    'Prefer titles such as "Устройство ...", "Монтаж ...", "Укладка ...", "Армирование ...", "Бетонирование ...", "Окраска ..." instead of "Устроить ...", "Смонтировать ...", "Уложить ...", "Армировать ...", "Забетонировать ...", "Покрасить ...".',
    'Task titles must read like standard construction work item names, not like instructions to an assignee.',
    'Bad task title examples: "Устроить цементную стяжку пола", "Уложить чистовое напольное покрытие", "Смонтировать окна".',
    'Good task title examples: "Устройство цементной стяжки пола", "Укладка чистового напольного покрытия", "Монтаж окон".',
...
export function buildSchedulingPrompt(input: SchedulingPromptInput): string {
  return [
    ...
    'Keep cross-phase and cross-section dependencies sparse.',
    'Between top-level phases or sections, add only key handoff dependencies that explain the main readiness logic.',
    'Aim for roughly 1-2 meaningful dependencies between neighboring top-level phases or sections, not a dense mesh.',
    'Do not weave many task-to-task links across different top-level phases or sections when one key gateway dependency is enough.',
    'Inside one phase or one section, you may keep more detailed task dependencies when they reflect real execution logic.',
    'Inside one phase or one section, avoid hanging tasks: each non-initial work task should usually have at least one meaningful predecessor from the same phase or section unless there is a clear reason not to.',
    ...buildLocationGranularityLines(input),
