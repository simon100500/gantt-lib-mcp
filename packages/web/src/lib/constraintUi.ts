...
export function buildConstraintModalContent(
  ...
  if (denial.code === 'SUBSCRIPTION_EXPIRED') {
    ...
    title = 'Портфели проектов недоступны';
    description = 'На бесплатном тарифе портфели проектов недоступны. Расширьте тариф, чтобы создавать отдельные портфели и работать по командам или направлениям.';
