...
}: RenderOptions = {}): { container: HTMLDivElement; root: Root; onSelectBaseline: (baselineId: string) => void; onHideBaseline: () => void; onRefreshBaselines: () => void } {
  ...
  act(() => {
    root.render(
      <>
        ...
        />
        {mobileMenuOpen ? (
          <div data-testid="force-mobile-visibility" className="lg:hidden sm:hidden" />
        ) : null}
      </>
    );
  });

  return { container, root, onSelectBaseline, onHideBaseline, onRefreshBaselines };
}

describe('Toolbar baseline menu', () => {
  it('renders the desktop trigger with the active baseline label and selected-row marker', () => {
    ...
    expect(container.textContent).toContain('Активный baseline');
    expect(container.textContent).toContain('Sprint plan v1');
    expect(container.textContent).toContain('Forecast copy');
    expect(container.textContent).toContain('Активный');
    expect(container.textContent).toContain('Скрыть baseline');
    expect(container.textContent).toContain('Обновить baseline-ы');

    const selectedRow = Array.from(container.querySelectorAll('[role="menuitem"]')).find((node) => node.textContent?.includes('Sprint plan v1'));
    ...
    const hideItem = Array.from(container.querySelectorAll('[role="menuitem"]')).find((node) => node.textContent?.includes('Скрыть baseline'));
    act(() => {
      hideItem?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
    expect(onHideBaseline).toHaveBeenCalledTimes(1);

    const refreshItem = Array.from(container.querySelectorAll('[role="menuitem"]')).find((node) => node.textContent?.includes('Обновить baseline-ы'));
    act(() => {
  ...
  it('renders explicit loading, error, and empty baseline states without crashing', () => {
    const loadingRender = renderToolbar({ baselineMenuOpen: true, baselineLoading: true, baselineRows: null, baselineError: null });
    expect(loadingRender.container.textContent).toContain('Загрузка baseline-ов…');
    ...
    expect(errorRender.container.textContent).toContain('Не удалось загрузить baseline-ы');
    ...
    expect(emptyRender.container.textContent).toContain('Сохранённых baseline-ов пока нет');
    expect(emptyRender.container.textContent).not.toContain('Скрыть baseline');
    emptyRender.root.unmount();
  });

  it('falls back safely when active label and props are malformed', () => {
    const { container, root } = renderToolbar({
      baselineMenuOpen: true,
      baselineActiveLabel: '   ',
      baselineRows: [
    ...
    expect(container.textContent).not.toContain('Активный baseline');
    expect(container.textContent).toContain('Без названия');
    expect(container.textContent).not.toContain('Скрыть baseline');

    root.unmount();
  });

  it('mirrors baseline actions in the mobile overflow menu', () => {
    const onSelectBaseline = vi.fn();
    const onHideBaseline = vi.fn();
    const onRefreshBaselines = vi.fn();
    ...
    expect(document.body.textContent).toContain('Скрыть baseline');
    expect(document.body.textContent).toContain('Обновить baseline-ы');

    const mobileBaselineItem = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((node) => node.textContent?.includes('Mobile baseline'));
    act(() => {
      mobileBaselineItem?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
    expect(onSelectBaseline).toHaveBeenCalledWith('baseline-1');

    const hideItem = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((node) => node.textContent?.includes('Скрыть baseline'));
    act(() => {
      hideItem?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
    expect(onHideBaseline).toHaveBeenCalledTimes(1);
