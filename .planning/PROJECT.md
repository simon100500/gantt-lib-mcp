...
**Goal:** Migrate from SQLite to PostgreSQL for production scalability with multiple concurrent users.

**Target features:**
- Replace @libsql/client with PostgreSQL client (pg or drizzle-orm)
- Update database schema for PostgreSQL compatibility
- Configure multi-instance deployment support
- Update deployment for external PostgreSQL database
- Fresh database start (no data migration from SQLite)
- Connection pooling for better performance

**Motivation:** Production scaling — better performance for multiple concurrent users than SQLite can provide.
...
- [ ] Postgres client integration (pg or drizzle-orm)
...
- SQLite persistent volume at /data
...
| PostgreSQL для production | Несколько контейнеров + concurrent users — SQLite bottleneck | — Pending (v2.0) |
