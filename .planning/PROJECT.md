...
**Status:** Phase 23 complete — Task Filtering UI shipped

**Goal:** Улучшить MCP-сервер: экономия токенов, надёжность агента, иерархия задач, история диалога

**Problems addressed:**
1. Нет истории диалога — модель не видит предыдущие сообщения
...
**Target features:**
- Compact mode в `get_tasks` с пагинацией
- Ограничение истории диалога (20 сообщений)
- Лимит ходов агента (20) и таймаут (2 мин)
- `parentId` в MCP инструментах (создание/обновление/фильтрация)
- Новые инструменты: `get_conversation_history`, `add_message`
- Улучшенные описания и сообщения об ошибках
...
- ✓ FILTER-01 through FILTER-04 — Task Filtering UI — v3.0 (Phase 23)

### Active (v3.0)

- [ ] TOKEN-01 through TOKEN-03 — Token Economy (compact mode, pagination, history limit)
- [ ] HARD-01 through HARD-03 — Qwen SDK Hardening (max turns, timeout, tool restrictions)
- [ ] HIER-01 through HIER-03 — Task Hierarchy (parentId in MCP tools)
- [ ] HIST-01 through HIST-02 — Conversation History (get_conversation_history, add_message)
- [ ] QUAL-01 through QUAL-02 — Tool Quality (descriptions, error messages)
...
---
*Last updated: 2026-03-20 after Phase 23 (Task Filtering UI) completion*
