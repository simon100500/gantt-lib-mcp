...
plan: "01"
subsystem: infra
tags: [monorepo, npm-workspaces, typescript, react, vite, fastify, mcp]
...
tech-stack:
  ...
  patterns:
    - npm workspaces monorepo with packages/* structure
    - Each package has its own tsconfig.json (no shared rootDir/outDir at root)
    - Vite proxy pattern for local dev (/api, /ws → localhost:3000)

key-files:
  created:
    - packages/mcp/package.json
    - packages/mcp/tsconfig.json
    - packages/mcp/src/ (all MCP sources)
    - packages/mcp/agent/ (agent sources with updated paths)
  ...
  modified:
    - package.json (added workspaces, monorepo scripts, removed src-level deps)
    - tsconfig.json (base config only, no rootDir/outDir)
    - .gitignore (added packages/*/dist/ and packages/*/node_modules/)
    - packages/mcp/agent/agent.ts (fixed PROJECT_ROOT and path references)
