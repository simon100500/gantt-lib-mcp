...
plan: "04"
subsystem: ui
tags: [react, dhtmlx-gantt, typescript, vite, gantt-chart]

# Dependency graph
requires:
  - phase: 07-01
    provides: monorepo scaffold with packages/web Vite+React app and Vite proxy to :3000
...
affects:
  - 07-05  # WebSocket wiring connects to setTasks from useTasks

# Tech tracking
tech-stack:
  added: [dhtmlx-gantt ^8.0.0]
  patterns:
    - useRef + useEffect for imperative DOM library integration
    - gantt.init() once on mount, gantt.clearAll() + gantt.parse() on task prop change
    - useTasks() hook with loading/error state, exposes setTasks for external mutation
